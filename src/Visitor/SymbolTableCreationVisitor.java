...
import AstGeneration.*;
import SymbolTable.*;

public class SymbolTableCreationVisitor implements Visitor {
    PrintWriter logger;

    public SymbolTableCreationVisitor(PrintWriter logger) {
        this.logger = logger;
    }
    ...
    public void visit(Node node) throws IOException {
         ...
         */
        if (node.getName().equals("PROG")) {
            node.symbolTable = new SymbolTable("global", null);
            // propagate accepting the same visitor to all the children, 
            // which effectively achieves Depth-First AST Traversal
            
            Node child = node.getLeftMostChild();
            
            while(child != null) {
            	// pass symbol table to children, so children can add their entry to the parent symbol table
            	child.symbolTable = node.symbolTable;
                child.accept(this);
                child = child.getRightSibling();
            }
        }else if (node.getName().equals("FUNCFEF")) {
         ...
         * | | | └──float
         */
        else if (node.getName().equals("STRUCTDECL")) {
            SymbolTable parentTable = node.symbolTable;
            ...
            for (Node i : inherlist) {
                node.symbolTableEntry.inherits.add(i.getToken().getName());
            }
        ...
        else if (node.getName().equals("FUNCHEAD")) {
         ...
         * | | | | | | └──indiceList
         */
        else if (node.getName().equals("IMPLDEF")) {
